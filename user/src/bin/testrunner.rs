...
fn run_task_a_mmap_phase5_probe() -> i32 {
    let pid = fork();
    assert!(pid >= 0, "failed to fork mmap_phase5_probe");
    if pid == 0 {
        let argv = ["mmap_phase5_probe\0".as_ptr(), core::ptr::null()];
        let ret = exec("mmap_phase5_probe\0", &argv);
        println!("[testrunner] exec mmap_phase5_probe failed: {}", ret);
        exit(-1);
    }
    ...
    assert_eq!(waitpid(pid as usize, &mut status), pid);
    if status == 0 {
        println!("[testrunner] mmap Phase 5 probe PASS");
    } else {
        println!("[testrunner] mmap Phase 5 probe failed: status={}", status);
    }
    status
}
...
fn run_task_a_mprotect_failure_probe() -> i32 {
    ...
    if pid == 0 {
        let argv = ["mprotect_failure_probe\0".as_ptr(), core::ptr::null()];
        let ret = exec("mprotect_failure_probe\0", &argv);
        println!("[testrunner] exec mprotect_failure_probe failed: {}", ret);
        exit(-1);
    }
    ...
    assert_eq!(waitpid(pid as usize, &mut status), pid);
    if status == 0 {
        println!("[testrunner] mprotect failure probe PASS");
    } else {
        println!(
            "[testrunner] mprotect failure probe failed: status={}",
            status
        );
    }
    status
...
fn run_task_a_sysv_shm_lifecycle_probe() -> i32 {
    let pid = fork();
    assert!(pid >= 0, "failed to fork sysv_shm_lifecycle_probe");
    if pid == 0 {
        let argv = ["sysv_shm_lifecycle_probe\0".as_ptr(), core::ptr::null()];
        let ret = exec("sysv_shm_lifecycle_probe\0", &argv);
        println!("[testrunner] exec sysv_shm_lifecycle_probe failed: {}", ret);
        exit(-1);
    }
    ...
    assert_eq!(waitpid(pid as usize, &mut status), pid);
    if status == 0 {
        println!("[testrunner] SysV SHM lifecycle probe PASS");
    } else {
        println!(
            "[testrunner] SysV SHM lifecycle probe failed: status={}",
            status
        );
    }
    status
...
fn run_task_a_sysv_shm_attach_race_probe() -> i32 {
    let pid = fork();
    assert!(pid >= 0, "failed to fork sysv_shm_attach_race_probe");
    if pid == 0 {
        let argv = ["sysv_shm_attach_race_probe\0".as_ptr(), core::ptr::null()];
        let ret = exec("sysv_shm_attach_race_probe\0", &argv);
        println!(
            "[testrunner] exec sysv_shm_attach_race_probe failed: {}",
            ret
        );
        exit(-1);
    }
    ...
    assert_eq!(waitpid(pid as usize, &mut status), pid);
    if status == 0 {
        println!("[testrunner] SysV SHM attach race probe PASS");
    } else {
        println!(
            "[testrunner] SysV SHM attach race probe failed: status={}",
            status
        );
    }
    status
...
fn run_task_a_pwrite_append_atomic_probe() -> i32 {
    ...
    if pid == 0 {
        let argv = ["pwrite_append_atomic_probe\0".as_ptr(), core::ptr::null()];
        let ret = exec("pwrite_append_atomic_probe\0", &argv);
        println!(
            "[testrunner] exec pwrite_append_atomic_probe failed: {}",
            ret
        );
        exit(-1);
    }
    ...
    assert_eq!(waitpid(pid as usize, &mut status), pid);
    if status == 0 {
        println!("[testrunner] pwrite append atomic probe PASS");
    } else {
        println!(
            "[testrunner] pwrite append atomic probe failed: status={}",
            status
        );
    }
    status
