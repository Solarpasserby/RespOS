...
use crate::arch::rv64::trap::sigreturn_trampoline;
use crate::mm::copy_to_user;
use crate::task::{current_task, exit_and_run_next};
use sig_handler::SigActionFlag;
    ...
    while let Some((sig, _siginfo)) = task.op_sig_pending_mut(|p| p.fetch_signal()) {
        ...
        if !action.is_user() {
            ...
            let mut user_sp = if action.flags.contains(SigActionFlag::SA_ONSTACK) {
                task.sigstack().map_or(trap_cx.get_sp(), |s| s.ss_sp)
            } else {
                trap_cx.get_sp()
            };
            ...
            copy_to_user(user_sp as *mut SigContext, &sig_ctx as *const SigContext, 1)
                .expect("[handle_signal] copy_to_user failed");
