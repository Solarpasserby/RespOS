...
impl HttpServer {
    ...
    pub fn new() -> Option<Self> {
        ...
        LISTEN_TABLE.lock().listen(endpoint, handle, HTTP_BACKLOG).ok()?;
        Some(Self {
            port: HTTP_PORT,
            conns: Vec::new(),
        })
...
impl HttpConn {
    ...
    fn poll(&mut self) -> bool {
        ...
        if self.response.is_none() {
            ...
            let (read, peer_closed) = socket_set()
                .lock()
...
fn status_html() -> String {
    ...
    let _ = writeln!(out, "<tr><td>物理内存总量</td><td>{}</td></tr>", fmt_bytes(mem_total));
    let _ = writeln!(out, "<tr><td>空闲内存</td><td>{}</td></tr>", fmt_bytes(mem_free));
    let _ = writeln!(
        out,
    ...
    let _ = writeln!(out, "<tr><td>内核堆占用</td><td>{}</td></tr>", fmt_bytes(heap_used));
    let _ = write!(out, "</table>\n</body></html>\n");
    out
}
