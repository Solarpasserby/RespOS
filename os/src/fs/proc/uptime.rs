...
use super::dirs::{proc_dev, proc_uptime_ino};
...
pub(super) struct UptimeInode;

impl InodeOp for UptimeInode {
    ...
    fn stat(&self, _path: &str) -> SysResult<KStat> {
        Ok(KStat::minimal(generate_uptime().len(), InodeType::Regular)
            .with_dev(proc_dev())
            .with_ino(proc_uptime_ino())
            .with_mode(0o444))
    }
...
fn generate_uptime() -> String {
    ...
    let _ = writeln!(
        ...
        "{}.{:02} {}.{:02}",
        uptime_us / 1_000_000,
        uptime_us % 1_000_000 / 10_000,
        idle_us / 1_000_000,
        idle_us % 1_000_000 / 10_000
    );
    result
}
