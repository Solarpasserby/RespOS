...
<9-硬件抽象层与架构适配>
#quote(block: true)[
本章回答：RespOS 如何在 RISC-V64 和 LoongArch64 上复用同一套内核逻辑？架构层如何统一页表、trap、任务切换、时钟和启动流程？
]
...
这种设计让 `MemorySet` 可以在两种架构上复用相同的 VMA、页帧和映射逻辑。用户地址空间的差异主要集中在页表项编码、地址转换寄存器和 TLB 刷新，而不会扩散到 COW、lazy allocation、file-backed mapping 等上层机制。
...
总体而言，RespOS 没有试图抹平 RISC-V 和 LoongArch 的硬件差异，而是把差异收敛到入口、寄存器、页表、trap 汇编和任务切换等少数边界。公共内核因此可以围绕统一的任务、地址空间和系统调用语义持续演进；架构层则负责将这些语义准确落实到两套不同的指令集和特权体系上。
