...
align(center)[#table(
  ...
  inset: 6pt,
  [机制], [内核中的核心对象], [是否传输数据], [何时进入阻塞], [典型用途],
  [信号],
  [每线程 `SigPending`、线程组共享 `SigHandler`],
  [`SigInfo` 与信号编号],
  [`sigsuspend`、`sigtimedwait` 或打断其他可中断等待],
  [异步通知、timer、异常、子进程状态变化],
  [管道],
  [共享 `PipeRingBuffer` 与读写端 `FileOp`],
  [有序字节流],
  [读空且仍有写端；写满且仍有读端],
  [shell 重定向、父子进程输出捕获],
  [futex],
  [`FutexKey`、哈希等待队列、完成状态],
  [用户数据留在共享内存，内核只管理等待关系],
  [用户值等于期望值且发生竞争],
  [pthread mutex/condvar、线程 join],
  [System V shm],
  [`ShmSegment`、共享 frame、每次 attach 记录],
  [进程直接读写同一物理页],
  [映射本身不阻塞],
  [大块共享数据、跨进程共享状态],
)]
...
align(center)[#table(
  ...
  inset: 6pt,
  [状态], [共享范围], [主要更新者], [生命周期边界],
  [pending + `SigInfo`],
  [每线程],
  [kill/tkill/tgkill、异常、timer、pipe、子进程事件],
  [投递或 `sigtimedwait` 消费；exec 清空],
  [signal mask],
  [每线程],
  [`sigprocmask`、`sigsuspend`、handler 进入与 sigreturn],
  [clone 继承当前 mask；sigreturn 恢复旧 mask],
  [alt stack],
  [每线程],
  [`sigaltstack`],
  [新线程使用默认值；exec 重置],
  [handler 表],
  [线程组共享],
  [`sigaction`、`SA_RESETHAND`、exec],
  [同组线程共同可见；exec 重置用户 handler],
)]
)
...
align(center)[#table(
  ...
  [任务进入 `Stopped`，父进程可等待该事件],
  [Continue],
  [`SIGCONT`],
  [将 stopped 任务重新发布为 `Ready`，并记录 continued 事件],
  [Term / Core],
  [`SIGTERM`、`SIGKILL` / `SIGSEGV`、`SIGABRT` 等],
  [进入线程组退出；Core 类暂不写 core 文件],
)]
)
...
align(center)[#table(
  ...
  [read 且缓冲区非空],
  [取出可用字节并返回],
  [唤醒一个 writer，通知 `POLL_WRITE`],
  [read、缓冲区空、仍有写端],
  [reader 登记并进入 `Blocked`],
  [等待 write、写端关闭或 signal],
  [read、缓冲区空、所有写端关闭],
  [返回 0（EOF）],
  [poll 可观察 HUP/可读结束状态],
  [write 且有空间、仍有读端],
  [写入可容纳字节并返回],
  [唤醒一个 reader，通知 `POLL_READ`],
  [write、缓冲区满、仍有读端],
  [writer 登记并进入 `Blocked`],
  [等待读取、读端关闭或 signal],
  [write 且所有读端关闭],
  [返回 `EPIPE`，`sys_write` 产生 `SIGPIPE`],
  [当前任务按信号规则处理],
)]
)
...
align(center)[#table(
  ...
  align: (col, row) => (auto,auto,auto,auto,).at(col),
  inset: 6pt,
  [映射类型], [`scope` 来源], [key 中的地址部分], [目的或边界],
  [`FUTEX_PRIVATE_FLAG`],
  [tgid],
  [原用户虚拟地址],
  [仅同一线程组内匹配],
  ...
  [物理 frame 身份],
  [VMA 页索引 + 页内偏移],
  [fork 继承或共享 frame 映射可匹配],
  [System V shm],
  [当前 attach id],
  [attach 内页索引 + 页内偏移],
  [同一次 attach 内稳定；不同进程独立 shmat 尚不能保证相同],
  [无法解析为共享 backing],
  [tgid],
  [原用户虚拟地址],
  [安全回退为进程内匹配],
...
align(center)[#table(
  ...
  align: (col, row) => (auto,auto,auto,auto,).at(col),
  inset: 6pt,
  [完成者], [目标状态], [队列与调度动作], [用户态结果],
  [`FUTEX_WAKE` / `WAKE_BITSET`],
  [`Woken`],
  [从哈希桶移除并唤醒 tid],
  [返回 0],
  [deadline],
  [`TimedOut`],
  [删除 queue/deadline 记录并唤醒],
  [`ETIMEDOUT`],
  [signal],
  [`Interrupted`],
  [删除 futex waiter，由 signal 路径唤醒 scheduler],
  [`EINTR`],
  [task exit],
  [不再恢复],
  [清理 queue、wait、deadline 三处记录],
  [无返回],
)]
)
...
align(center)[#table(
  ...
  [操作], [主要检查], [成功后的状态变化],
  [`shmget`],
  [key/IPC\_CREAT/EXCL、大小、权限、全局页数与段数上限],
  [创建或返回 `ShmSegment`，新段一次性分配所需 frames],
  [`shmat`],
  [shmid、访问权限、地址对齐与已知 flag],
  [在当前 `MemorySet` 映射共享 frames，登记 attach owner，更新 atime/lpid],
  [`shmdt`],
  [地址对齐且属于某个 shm attach],
  [删除该 attach，更新 dtime/lpid，必要时完成延迟删除],
  [`shmctl(IPC_RMID)`],
  [owner 或特权权限],
  [无 attach 时删除；否则标记 `SHM_DEST`],
  [`shmctl(SHM_LOCK/UNLOCK)`],
  [owner 或特权权限],
  [记录 locked 状态和 ctime],
)]
)
...
align(center)[#table(
  ...
  [机制], [依赖的内核边界], [已形成的能力],
  [信号],
  [TCB 的线程组/可中断状态、trap context、scheduler wakeup],
  [异步产生、线程选择、handler frame、默认动作与 sigreturn 完整闭环],
  [管道],
  [`FdTable`/`FileOp` 引用生命周期、blocked/ready 调度、poll waiters],
  [阻塞与非阻塞字节流、EOF/EPIPE/SIGPIPE、poll/epoll 通知],
  [futex],
  [`MemorySet` backing 身份、哈希等待队列、timer 与 signal],
  [wait/wake/bitset/requeue、超时和 signal 的 single-winner 竞争处理],
  [System V shm],
  [frame 所有权、VMA/PTE 建图、全局段表],
  [shmget/shmat/shmdt/shmctl、共享物理页、失败回滚与延迟删除],
)]
)
