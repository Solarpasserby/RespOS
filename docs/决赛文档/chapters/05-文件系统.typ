...
<5-文件系统>
#quote(block: true)[
本章介绍文件描述符、路径、目录项和 inode 之间的关系，并说明 ext4、procfs、devfs、页缓存和文件映射如何接入这套模型。
]
...
`FileOp` 的意义在于抽象出了如何使用文件的接口，而不关注其存储形式。因此 pipe 和 socket 也可以通过同一接口接入 `read`、`write`、poll 和 close；普通文件则在 `FileOp` 实现内部继续调用 `InodeOp`、`Path` 和 `PageCache`。

=== 5.1.3 `InodeOp`：文件对象的后端能力边界
<513-inodeop文件对象的后端能力边界>
`InodeOp` 解决的是对文件对象的解释和后端操作的关联。它不直接对应用户态 fd，而是提供节点类型、属性、按偏移 I/O、截断、目录查找、创建、链接、删除和可选的页缓存等能力：
...
当前接口中的 `path` 参数来自 lwext4 的路径型访问方式。VFS 通过 `Path` 和 `Dentry::current_abs_path()` 生成当前后端路径，再由具体 inode 实现把统一操作转换成底层调用。这个参数是当前 ext4 适配层需要的实现细节。
...
这些字段分别承担四类职责：`ino` 和 `ty` 标识后端对象；时间、权限、所有者和链接数覆盖字段保存 VFS 需要但当前后端不能直接稳定提供的元数据；`orphan_path` 与 `renamed_path` 处理路径变化期间仍然存活的 inode；`open_files` 区分真正的打开实例数量；`page_cache` 使同一个 inode 支持多个 `File` 共享文件数据缓存。

ext4 inode 的创建经过 inode cache：`get_or_create` 先按 inode 号查找全局缓存，已有 `Arc<Ext4Inode>` 仍被使用时，新的路径查找会复用同一 inode 对象，所有打开实例也就共享同一份页缓存和内核侧元数据。

具体操作需要同时处理 VFS 语义和 lwext4 的错误、锁与路径约束。`lookup` 通过 ext4 目录遍历获得子 inode 号和类型，再调用 `get_or_create`；`read_at`、`write_at`、`stat` 和 `truncate` 使用 `storage_path` 选择当前后端路径，创建临时 `Ext4File` 完成底层操作。对共享的 lwext4 入口，适配层使用 `EXT4_OP_LOCK` 串行化 FFI 调用，避免底层全局状态被并发破坏。
...
`Path` 是 `(VfsMount, Dentry)` 二元组，表示一个对象在整个挂载命名空间中的位置；`Dentry` 维护名字、父目录、子目录缓存等：
...
当前初始化流程把 ext4 根文件系统作为主存储后端，再将 procfs 和 devfs 以独立的 `VfsMount` 挂载到 `/proc` 和 `/dev`。tmpfs/shm 等接口也通过 VFS 和特殊文件对象提供相应语义。

=== 5.2.2 namei 路径遍历
<522-namei-路径遍历>
路径查找由 `Nameidata` 驱动。它保存当前挂载实例、当前 dentry、待处理的路径分量和遍历深度。相对路径从当前工作目录或 `dirfd` 对应的目录开始，绝对路径从任务 root 开始；`dirfd`、空路径、符号链接跟随等路径操作选项由 syscall 层传入并在 namei 中解释。
...
路径修改、后端操作和 dentry 缓存维护因此处于同一条可控的更新路径，create、unlink 和 rename 的可观察结果更容易保持一致。这也是当前适配路径型 lwext4 接口时的取舍：先保证命名空间状态和失败回滚，再根据实际并发负载细化锁粒度。
...
这一适配层还补齐了 lwext4 不直接替 VFS 管理的语义：`open_files` 单独记录 `File` 数量；普通 ext4 inode 通过 `get_page_cache` 接入共享页缓存；rename 后通过 `renamed_path` 保证旧的打开对象继续访问正确的后端路径；unlink 后通过 orphan 路径延迟清理，使名字从目录树中消失后，已有 fd 仍能继续访问文件。
...
pipe 不依赖磁盘 inode。两个端点共享一个带有读写关闭状态、FIFO 等待队列和 poll waiter 的环形缓冲区；当没有读者或写者时，端点生命周期决定 EOF 和 `SIGPIPE` 等可观察行为。socket、eventfd、epoll、timerfd 等对象也直接以 `FileOp` 形式进入 `FdTable`。这些对象复用文件描述符、poll 和 close 机制，但不套入 ext4 的页缓存或路径生命周期。
...
在一次普通文件访问中，fd 先定位到打开对象，打开对象持有路径，路径连接挂载实例和 dentry，dentry 再通过 `InodeOp` 调用具体文件系统。沿着这条调用链，系统同时支持磁盘文件、虚拟文件、设备文件和无路径特殊文件，缓存、同步、内存映射和进程生命周期也都能接入文件对象模型。

这套分层保留了 Linux VFS 模型的主要语义，也利用 Rust 的 `Arc`、trait 和错误传播机制管理对象生命周期、后端替换和失败路径。
