...
<4-内存管理>
#quote(block: true)[
本章回答：物理内存如何分配？进程地址空间如何表示？缺页、COW 与文件映射如何在同一套模型中协作？多核下又如何保证页表修改可见？
]
...
#strong[图 4-1 `MemorySet`、VMA、PTE 与驻留页的层次关系]

表 4-1 进一步给出四类对象的职责和生命周期。其中，VMA 是内核判定缺页能否被修复的依据，而不是仅用来加速查找的缓存。
...
#strong[代码片段 4-2 `FrameTracker` 的 RAII 回收]
...
#strong[图 4-2 RV64 可达范围与可分配范围的分离]
...
#strong[图 4-3 `handle_page_fault` 决策流程]
...
#strong[代码片段 4-3 COW 缺页的单所有者与多所有者分流]
...
#strong[图 4-5 RV64 共享地址空间的定向 TLB shootdown]
...
综上，RespOS 已将"分配一页内存"扩展为跨越进程、文件、trap 和多核调度的地址空间协议：虚拟范围可以延迟承诺，但权限必须在 VMA 和 PTE 两层一致；物理页可以共享，但所有权和回收必须有唯一落点；写回和页表替换可以分阶段执行，但失败不能留下半完成状态。
