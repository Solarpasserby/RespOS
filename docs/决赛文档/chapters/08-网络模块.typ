...
<8-网络模块>
#quote(block: true)[
本章回答：RespOS 如何将 smoltcp 协议栈接入 Linux socket ABI？TCP、UDP 和 UNIX 域 socket 的生命周期如何管理？并发 `listen/accept` 为什么需要专门的 listener 池？
]
...
这一设计的目标是满足只需本地回环的 libc 和应用探测，而不是宣称已建立 IPv6 接口地址、邻居发现和路由。虽然 smoltcp 依赖启用了 `proto-ipv6` 特性，可编译的第三方能力不自动等于本项目已将其连接到设备、地址配置和 ABI 的完整路径。
...
综上，RespOS 没有将第三方协议库的可编译功能简单等同于项目成果，而是以真实调用链和用户可见语义划定边界。当前网络模块已经是一条闭环的本地通信路径：套接字能被 fd 拥有、被 smoltcp 驱动、在阻塞时与调度/信号协作、在并发握手时维持 backlog 容量，并在 close 后把协议句柄归还唯一所有者。
