...
<2-进程管理>
#quote(block: true)[
本章重点介绍 RespOS 的任务模型，以及进程和线程在创建、调度、退出和多核切换中的处理方式。
]
...
这个模型需要同时处理几个实际问题：任务对象如何持有资源，`clone` 如何组合共享语义，阻塞与唤醒如何避免丢失事件，任务切换如何保证旧上下文已经保存，以及退出任务如何在不释放当前内核栈的前提下完成回收。表 2-1 对应列出了这些问题的处理方式。
...
align(center)[#table(
  ...
  inset: 6pt,
  [设计目标], [RespOS 的实现], [产生的效果],
  [统一进程与线程],
  [TCB + tid/tgid + `ThreadGroup`],
  [创建、调度、阻塞、信号和退出复用同一套任务接口],
  [明确资源共享边界],
  [`Arc<MemorySet>`、`Arc<FdTable>`、共享 handler 表],
  [`clone` 可按 flag 选择共享或复制，fork/线程语义不混用],
  [保证阻塞唤醒正确],
  [ready/blocked 分离、事件单赢家、发布后复查],
  [覆盖 wait、futex、pipe 和 signal 的 lost-wakeup 窗口],
  [保证多核切换安全],
  [per-CPU idle context、handoff slot、`cpu_owner`],
  [其他 CPU 不会恢复尚未保存完成的任务上下文],
  [分离退出与析构],
  [Exited 状态、父进程强引用、`DEAD_TASKS`],
  [wait status 可保留，TCB 与内核栈在安全栈上延迟释放],
)]
)
...
align(center)[#table(
  ...
  inset: 6pt,
  [对象或状态], [创建者与共享范围], [更新/释放方式], [用户可见语义],
  [`KernelStack`、trap context、`TaskContext`],
  [每个 TCB 独占],
  [CPU 切到 idle 栈后随 TCB 延迟析构；slot 可复用],
  [每个线程可独立陷入内核和被调度],
  [`ThreadGroup`],
  [同组线程共享，成员表保存 `Weak<TCB>`],
  [创建线程时加入，线程退出或 exec 清理时移除],
  [tid 不同但 tgid 相同，组级操作作用于全部线程],
  [`MemorySet`],
  [fork 默认建立 COW 子空间；`CLONE_VM` 可共享],
  [exec 替换，最后一个所有者退出时回收],
  [决定进程间内存隔离或线程间地址空间共享],
  [`FdTable`],
  [fork 复制描述符表；`CLONE_FILES` 或线程创建时共享],
  [close/exec/exit 更新；exec 先私有化再处理 CLOEXEC],
  [决定一方 close 是否影响另一方的 fd 表],
  [`SigPending`、mask、备用信号栈],
  [每线程独占],
  [投递、sigprocmask、sigreturn 和 exec 更新],
  [每个线程可独立屏蔽、等待和处理信号],
  [`SigHandler`],
  [同一线程组共享],
  [`sigaction` 更新，exec 重置用户 handler],
  [任一线程修改 handler 后全进程可见],
  [`TASK_MANAGER`],
  [全局 `tid -> Weak<TCB>`],
  [创建时加入，逻辑退出时移除],
  [kill/tkill/tgkill、调度和清理路径可按 tid 定位任务],
  [`children`],
  [线程组共享，保存子进程 leader 的 `Arc<TCB>`],
  [子进程创建时加入，`wait4` 成功后删除],
  [zombie 状态在父进程读取前不会丢失],
)]
)
...
align(center)[#table(
  ...
  [标志], [未设置], [设置后], [当前实现边界],
  [`CLONE_THREAD`],
  [新 tid 同时成为新 tgid，创建新线程组],
  [新 tid 加入调用者线程组并沿用 tgid],
  [要求 `CLONE_SIGHAND`，后者又要求 `CLONE_VM`],
  [`CLONE_VM`],
  [通过 COW 建立子 `MemorySet`],
  [共享同一个 `MemorySet`],
  [非线程 vfork 为保护父映像，当前仍建立 COW 子空间],
  [`CLONE_FILES`],
  [新进程复制 descriptor 表],
  [共享同一个 `FdTable`],
  [同组线程无条件共享 fd table],
...
align(center)[#table(
  ...
  [阶段], [主要工作], [正确性保证],
  [准备新映像],
  [校验 ELF，创建新 `MemorySet`，布置 argv/envp/auxv],
  [失败发生在提交前，旧程序仍保持完整],
  [静止 sibling],
  [设置 `terminate_requested`，从调度器摘除并等待 `cpu_owner` 释放],
  [其他 CPU 不再执行旧用户映像],
  [提交映像],
  [替换 `MemorySet`、激活页表、重建 trap context],
  [PC、SP、寄存器与新地址空间同时生效],
  [提交外围状态],
  [私有化 fd table、执行 CLOEXEC、重置信号状态、释放 vfork parent],
  [父任务只会在 exec 可见状态完整后恢复],
)]
)
...
align(center)[#table(
  ...
  [线程逻辑退出],
  [当前 TCB 变为 `Exited`],
  [清理 robust list、clear-child-tid、futex waiter，并从调度器/线程组/任务索引摘除],
  [线程组退出],
  [原子选出唯一 teardown owner],
  [静止 sibling，处理共享 MM/fd/timer，托管孤儿，leader 向父进程发布 SIGCHLD 与 wait status],
  [`wait4` 成功],
  [父任务删除 `children[child_tid]`],
  [先 copyout status/rusage，成功后再删除 zombie 并累计子进程时间],
  [安全栈上析构],
  [idle loop 清空 `DEAD_TASKS`],
  [最终 drop TCB，回收内核栈 slot；已有内核页表映射保留供复用],
)]
)
...
align(center)[#table(
  ...
  align: (col, row) => (auto,auto,auto,auto,).at(col),
  inset: 6pt,
  [任务类型], [队列组织], [选择顺序], [当前语义边界],
  [`SCHED_FIFO` / `SCHED_RR`],
  [优先级 1～99 的 RT 队列],
  [数值越大越优先，同级 FIFO],
  [周期时钟当前会重新排队两者，尚未完整区分 Linux FIFO 与 RR],
  [`SCHED_OTHER` / `SCHED_BATCH`],
  [nice -20～19 的 40 个普通队列],
  [nice 越小越优先，同级轮转],
  [固定优先级模型，不维护 CFS 虚拟运行时间],
  [`SCHED_IDLE`],
  [单独的 idle 队列],
  [RT 与普通队列均无可运行任务时选择],
  [用于最低优先级后台工作],
...
align(center)[#table(
  ...
  align: (col, row) => (auto,auto,auto,).at(col),
  inset: 6pt,
  [协作模块], [进程管理提供或依赖的接口], [已形成的能力],
  [内存管理],
  [`MemorySet` 的 COW clone、exec 替换、active-hart 标记与回收],
  [fork 延迟复制、文件 ELF 按需加载、SMP 地址空间安全切换],
  [文件系统],
  [`FdTable` 复制/共享、CLOEXEC、退出清表],
  [fork 共享 open-file 状态，exec 不误关共享父表，pipe EOF 可按最后引用产生],
  [信号],
  [每线程 pending/mask、组共享 handler、可中断等待],
  [kill/tkill/tgkill、停止/继续/致命信号与 wait 唤醒接入统一 TCB],
  [futex/pipe/wait],
  [blocked 注册、single-winner 完成、signal interrupt],
  [事件、超时、signal 和退出竞争时不重复唤醒或永久睡眠],
  [时钟与 SMP],
  [抢占入口、per-CPU idle、affinity IPI、`cpu_owner`],
  [分层调度、跨核唤醒和 context handoff],
)]
)

在 RespOS 中，进程管理并不是一组彼此独立的系统调用。tid/tgid 负责定义身份，`Arc`/`Weak` 负责表达所有权，scheduler 负责发布状态，handoff/owner 负责划定多核切换边界，wait 与 `DEAD_TASKS` 分别处理用户可见的回收和内核对象析构。fork/exec/exit、信号和 IPC 都建立在这套任务模型上。
