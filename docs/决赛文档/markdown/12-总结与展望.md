...
| 文件系统 | 单 ext4 超块实例，无多设备支持；写密集场景下 NAMEI_MUTATION_LOCK 串行化；全路径存储 dentry 模型 rename 和缓存淘汰有额外开销；页缓存使用 Vec<u8> 而非物理页框，mmap 路径有多余拷贝 |
...
1. **BuildStorm 完整通过**。当前最重要的未闭环问题，涉及 pipe 引用生命周期、wait/wakeup 竞争、exit 延迟回收的综合调试
2. **LA64 多核验证**。当前 LA64 仍走单核路径，需要完成 LA64 的 SMP 启动、IPI 和共享 MM 验证
3. **内存管理完善**。swap / 页面回收 / 动态链接器 file-backed 加载
4. **性能优化**。per-CPU run queue、锁竞争优化、页缓存零拷贝 mmap
5. **功能补全**。`SA_RESTART`、消息队列、物理网卡驱动
