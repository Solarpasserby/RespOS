...
截至最终测试版本，RespOS 已经形成较完整的双架构宏内核主体。`os/src/syscall/mod.rs` 中维护 216 个 Linux syscall 分发入口，具体实现分布在 `fs.rs`、`process.rs`、`mm.rs`、`time.rs`、`net.rs`、`ipc.rs`、`signal.rs`、`special_fd.rs` 和 `system.rs` 等文件中；其中 `fs.rs` 与 `process.rs` 承担最多入口，分别覆盖文件/目录/fd/IO 语义和进程、调度、凭据、futex 等语义。后续优化重点已经从“补齐主路径”转向“收敛边界行为、提升双架构一致性和长测稳定性”。
...
#figure(
  ...
  #table(
    ...
    inset: 7pt,
    table.header([源码文件], [入口数量], [主要覆盖范围]),
    [`syscall/fs.rs`], [73], [文件读写、向量 IO、splice/sendfile、openat/openat2、stat/statx、权限、目录、链接、挂载、poll/select 等。],
    [`syscall/process.rs`], [56], [进程退出、调度属性、clone/execve/wait、pid/tid、futex、robust list、凭据和资源限制等。],
    [`syscall/special_fd.rs`], [20], [eventfd、epoll、inotify、signalfd、timerfd、pidfd、memfd、io_uring/bpf/fsopen 等兼容入口。],
    [`syscall/net.rs`], [18], [socket、bind/listen/accept/connect、send/recv、socket option、shutdown 等。],
    [`syscall/time.rs`], [17], [gettimeofday、clock_gettime、nanosleep、interval timer、POSIX timer 和时间校准接口。],
    [`syscall/signal.rs`], [12], [sigaction、sigprocmask、sigtimedwait、sigqueue、sigaltstack、sigreturn 等。],
    [`syscall/mm.rs` / `ipc.rs` / `system.rs`], [20], [mmap/brk/mprotect、SysV 共享内存、uname/sysinfo/reboot/personality 等。],
  )
]
...
从测试角度看，项目已经把测试流程从“手工观察串口输出”推进到“脚本化生成本地报告”。`rv-output.txt` 和 `la-output.txt` 是 2026 年 6 月 30 日的 RISC-V 64 与 LoongArch 64 全量测例输出；`judge/local-report/` 保存四组 LTP 逐用例统计 CSV；`judge/local-compare/` 保存与 Linux baseline 的逐条对比 CSV。下表记录本地报告 `TOTAL` 行统计，它反映最终测试版本的阶段性回归状态，不代表最终比赛排名。

#figure(
  kind: table,
  supplement: [表],
  caption: [本地 LTP 全量报告统计],
)[
  #table(
    ...
    inset: 7pt,
    table.header([平台/ libc], [TPASS], [TFAIL], [TBROK], [TCONF], [WARN], [总断言], [通过率]),
    [RISC-V + glibc], [4992], [54], [11], [90], [4], [5151], [96.91%],
    [RISC-V + musl], [5005], [26], [12], [93], [1], [5137], [97.43%],
    [LoongArch + glibc], [5054], [55], [9], [64], [3], [5185], [97.47%],
    [LoongArch + musl], [5035], [28], [11], [82], [1], [5157], [97.63%],
  )
]

按架构合并两个 libc 组合后，LoongArch 合计通过 10089 / 10342 条断言，通过率 97.55%；RISC-V 合计通过 9997 / 10288 条断言，通过率 97.17%。两套架构的差距主要来自少数文件系统与 I/O 边界测例，而不是启动、trap、页表或基本进程模型的系统性差异。glibc 组合的失败和 broken 数略高于 musl，说明剩余问题更多集中在更严格的 libc 行为、文件系统边界和脚本类测例，而不是单一架构不可用。

#figure(
  kind: table,
  supplement: [表],
  caption: [与 Linux baseline 的主要差异],
)[
  #table(
    columns: (auto, auto, auto, 1fr),
    align: (center + horizon, center + horizon, center + horizon, left + horizon),
    inset: 7pt,
    table.header([对比项], [RISC-V], [LoongArch], [说明]),
    [compare 记录数], [105 / 105], [107 / 119], [分别对应 glibc / musl compare CSV 中与 Linux baseline 不一致或缺失的用例记录。],
    [lost_passed 合计], [766 / 761], [715 / 738], [数值表示相对 Linux baseline 少通过的断言数量；RISC-V glibc 最高，LoongArch glibc 最低。],
    [主要 missing], [`tar_tests.sh`、`creat09`、`file01.sh`], [`tar_tests.sh`、`creat09`、`file01.sh`], [脚本类、文件创建和文件工具类测例仍是最大空缺来源。],
    [主要 failed / partial], [`splice07`、`chdir01`、`fsync01`、`rename01/03`], [`chdir01`、`fsync01`、`rename01/03`], [差异集中在 splice、目录切换、同步写回、重命名和目录项更新等文件系统与 I/O 语义。],
  )
]

这些数字需要结合测例列表和日志解释：有些失败来自尚未实现的边界语义，有些来自 libc 行为差异，有些则是比赛收益较低或运行时间较长的测例被主动跳过。文档前面的“初赛进展与排名”保留最终截图 TODO，本章只总结当前工程和本地验证已经覆盖的范围。

总体来看，RespOS 初赛阶段最大的收获不是某一个单点功能，而是建立了可以持续扩展的内核骨架。后续新增系统调用或修复测例时，通常能落到明确的位置：用户指针与错误码在 syscall 层处理，资源对象通过 fd/VFS/TCB/MemorySet 管理，架构差异由 `arch` 层兜底，测试结果由本地报告和对比脚本回归。当前剩余差距也说明后续优先级应放在文件系统写回、目录项更新、splice/sendfile、权限与路径错误码等可定位边界上。
...
#figure(
  ...
  #table(
    ...
    [文件系统差异收敛], [优先处理 `tar_tests.sh`、`creat09`、`file01.sh`、`chdir01`、`fsync01`、`rename01/03` 和 `splice07` 暴露出的目录、创建、同步写回、重命名和 splice 语义。], [直接降低 compare CSV 中 lost_passed 最高的一组差异。],
    [ABI 细节补齐], [继续完善 `openat/openat2`、权限检查、xattr、mount、socket option、SysV IPC、资源限制和特殊 fd 行为。], [减少 LTP 中因错误码、标志位和特殊路径不一致导致的失败。],
    [内存与进程生命周期], [继续审查 `fork`/`clone`/`execve`/`exit`/`wait4` 与 COW、mmap、futex、robust list 的交互。], [减少短进程密集创建、线程退出和共享内存场景中的状态残留。],
    [双架构一致性], [把 RISC-V 与 LoongArch 的 trap、时钟、页表、TLB、用户 ABI 和设备路径做成固定对照回归。], [避免一个平台修复后另一个平台退化，提高双架构提交质量。],
    [网络与设备扩展], [在当前回环网络基础上评估 VirtIO-net；完善 loop、RTC、random、tty 等设备文件语义。], [支持更完整的用户态工具链和网络/设备类测例。],
    [测试基础设施], [将全量输出、报告生成、baseline 对比、失败分类和耗时统计纳入固定流程，并保留关键失败日志。], [让每次修改都能快速判断收益和回归风险。],
...
#summary-box(
  [本章小结],
  [RespOS 初赛阶段已经形成可运行、可测试、可继续扩展的双架构宏内核主体。最终测试版本在四组 RISC-V/LoongArch 与 glibc/musl LTP 组合中均达到约 97% 的断言通过率；当前最重要的后续工作，是在已有模块边界内继续收敛文件系统、I/O 和 Linux ABI 细节，提升双架构一致性，并把本地 LTP 报告转化为稳定的回归流程。开源项目借鉴、AI 协作和许可证说明不在本章展开，统一放入第十二章。],
  fill: rgb("#F8FAFC"),
  accent: rgb("#667085"),
)
