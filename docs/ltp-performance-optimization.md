...
testrunner 会从 `/proc/respos_health` 读取低成本内核快照。正常运行时只在 libc
组开始、phase 边界和每 25 个 case 输出；单个 case 资源变化异常、fork/wait
失败或非零且非 `TCONF(32)` 退出时会立即输出。

```text
LTP HEALTH reason=periodic group=ltp-musl seq=25 case=... \
free_kb=... free_delta=... cached_kb=... cache_delta=... \
heap_kb=... heap_delta=... tasks=... task_delta=... \
...
- `free_kb`：空闲物理页容量；
- `cached_kb`：文件页缓存容量，可用于解释正常的 `free_kb` 下降；
- `heap_kb`：内核堆当前已分配容量；
- `tasks`：任务管理器中的任务数；
- `ready/blocked`：调度器就绪与阻塞任务数；
- `deferred`：等待离开当前内核栈后释放的任务数。

异常规则保持保守：

- 排除页缓存增长后，单 case 空闲内存下降超过 2 MiB；
- 单 case 内核堆增长超过 2 MiB；
- case 边界的 task、ready、blocked 或 deferred 数量增长；
- case 执行失败。

失败时还会输出 `LTP CASE DIAG`，保留 `pid`、`waited`、原始 wait status、
归一化返回码与耗时。若日志停在 `RUN LTP CASE`，说明父进程仍未从 wait
返回；此前最近一条 `LTP HEALTH` 可作为卡死前的资源基线。
