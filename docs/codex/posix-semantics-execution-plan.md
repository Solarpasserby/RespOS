...
| TCP `shutdown(SHUT_WR)`/half-close | Linux/RV64/LA64 probe 覆盖排队数据先于 EOF、反向流、dup 后 `EPIPE`、poll EOF readiness，以及 buffered FIN 的 `POLLRDHUP/EPOLLRDHUP` | 双架构已验证（loopback TCP 当前范围） | 补 AF_UNIX RDHUP、跨线程阻塞唤醒、reset/linger 与非 loopback |
