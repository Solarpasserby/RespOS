...
| SysV SHM `shmat/shmdt` | 跨 attach 数据/futex、`IPC_RMID` lifecycle、`shm_nattch` MM identity，以及 `shmat` 与最后 detach/RMID 的发布竞态和失败回滚 probe 均在 Linux/RV64/LA64 通过；竞态门禁覆盖 32 轮双 attacher（64 次 attach）和 128 轮顺序回收循环；默认 `SHMMNI=4096` 的顺序耗尽、`ENOSPC`、释放后重建和配额清理已通过；非空地址已按精确映射处理；`shmctl01` 的 12 项断言在 RV/LA 正确，但 LA64 卡在 20-child signal/reap teardown；LA64 glibc 2.38 仍有旧 64 KiB SHMLBA 冲突 | 双架构已验证（当前共享/生命周期/计数/双 attacher 线性化、顺序回收与 SHMMNI 范围）；LA teardown 与 runtime 已知差异 | 待协商：LA 多子进程 signal/exit/wait 活性；另补更宽 N 路并发、`SHM_REMAP` 并发、动态 sysctl、`SHMALL`/物理内存/ID 溢出资源边界与其余 metadata；runtime 更新 |
