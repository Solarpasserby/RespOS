...
| TCP `shutdown(SHUT_WR)`/half-close | Linux/RV64/LA64 probe 覆盖排队数据先于 EOF、反向流、dup 后 `EPIPE` 和 poll EOF readiness | 双架构已验证（loopback 基础范围） | 补 `POLLRDHUP`、跨线程阻塞唤醒、reset/linger 与非 loopback |
