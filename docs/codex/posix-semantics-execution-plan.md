...
| ext4 特殊 inode、`mknod` device payload 与 xattr 限制 | 双架构 probe 验证四类 inode mode、低位 rdev 与 `EPERM`；musl/glibc 13-case mknod/xattr 簇通过 | 双架构已验证（当前范围） | 补大 major/high minor 的 `dev_t`/`statx`；设备驱动语义另立子项 |
