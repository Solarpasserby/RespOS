...
逐项确认以下接口是否只是分配通用 `SpecialFd`（结论见
[C 组 Day1 审计台账](./四天内核重构-C-Day1-审计台账.md)）：
...
- [x] `epoll_pwait` 是否忽略 signal mask 和 sigset size；
- [x] 临时 signal mask 是否在等待期间原子替换并恢复；
- [x] edge/oneshot 状态是否在 events copyout 失败前被消费；
- [x] epoll interest 是 fd 数字、打开文件对象还是二者组合；
- [x] close 后 fd 复用是否会错误 DEL/MOD 旧 interest；
- [x] epoll 持锁注册底层 waiter 的锁顺序；
- [x] timerfd 接受 `CANCEL_ON_SET` 后是否真的取消；
- [x] alarm clock 是否具备对应语义；
- [x] memfd_create 接受 HUGETLB flags 后是否真实支持；
- [x] eventfd/epoll/timerfd 的 owner exit、close 和最后引用释放。

### 4.4 文件 ABI 红名单

- [x] pread/pwrite/preadv/pwritev 是否临时修改共享 offset；
- [x] splice 的显式 offset 是否也通过 seek/restore 模拟；
- [x] fsync/fdatasync/sync_file_range/msync 是否返回假成功；
- [x] F_SETFL 修改的是 fd entry 还是共享 open-file status；
- [x] rename/unlink 失败是否已经污染 dentry/cache；
- [x] mount/new mount API 是否只返回空壳对象；
- [x] flags 是否因截断或默认分支被静默接受；
- [x] 对错误 FileOp 类型是否返回正确 errno。
...
- [x] ADD → close target → fd number reuse；
- [x] ONESHOT + events copyout EFAULT；
- [x] ET 从 ready 到 not-ready 再 ready；
- [x] pwait 有/无 signal mask；
- [ ] waiter、timeout、signal 竞争。

### 4.6 第一天交付

- [x] FS/special-fd syscall 分级表；
- [x] 空壳 fd 的批量降级清单；
- [x] 5～10 份高风险状态影响审计；
- [x] epoll 和 sync 的失败注入计划；
- [x] fd/open-file identity 与生命周期图；
- [ ] 与 A/B 确认 waiter 和文件 mmap 接口。

前五项交付统一记录在
[C 组 Day1 审计台账](./四天内核重构-C-Day1-审计台账.md)；跨组接口契约已经提出，
等待 A/B owner 确认。
...
- [x] `pread64` 使用 `read_at_offset`；
- [x] `pwrite64` 使用 `write_at_offset`；
- [x] `preadv` 使用局部 positioned offset；
- [x] `pwritev` 使用局部 positioned offset；
- [x] 不调用 `seek`；
- [x] 不读取/恢复共享 offset；
- [x] 每段后 checked-add 局部 offset；
- [x] 保留短读、短写和部分成功语义；
- [x] `preadv2/pwritev2` 的 `offset == -1` 仍走普通共享 offset；
- [x] 不支持的 flags 返回明确错误。
...
- [x] pread/pwrite 不改变共享 offset；
- [x] sync 路径不再 panic；
- [x] fd/open-file flag 表完成；
- [x] 6 个以上文件回归；
- [ ] 提交按“positioned I/O”“sync”“fd flag”拆分。

实现、支持边界和双架构回归记录见
[C 组 Day2 实现与回归](./四天内核重构-C-Day2-实现与回归.md)。当前未代替负责人创建提交，
因此最后一项保留未勾选。
...
- [x] dirty page 写回后只有版本未变化才清 dirty；
- [x] writeback 失败保留 dirty；
- [x] fsync 覆盖所有目标 dirty page；
- [x] fdatasync 不承诺未实现的完整元数据语义；
- [x] truncate 缩小时清理尾部 cache；
- [x] 扩展文件时新区域读取为零；
- [x] close 不错误丢弃仍共享的 cache；
...
- [x] 至少修复一处明确的持 spin lock 慢 I/O；
- [x] dentry 生命周期测试；
- [x] page cache dirty/writeback 测试；
- [ ] 修改前后文件性能数据；
- [ ] 与 A/B 完成接口核对。

实现、锁盘点、双架构回归和修改后基准见
[C 组 Day3 锁、缓存与路径一致性](./四天内核重构-C-Day3-锁缓存路径一致性.md)。当前没有可复现的
修改前同配置基准，也未获得 A/B owner 的接口确认，因此最后两项保持未勾选。
...
- [x] 文件映射切分后 backing offset；
- [x] private/shared 语义；
- [x] page fault 是否在持 MM 全局锁时做慢 I/O；
- [x] munmap/msync 是否可能丢 dirty page；
- [x] truncate 与已映射页面是否至少行为明确。

审查结论与冻结边界见
[C 组 Day4 ABI 冻结与交叉审查](./四天内核重构-C-Day4-ABI冻结与交叉审查.md)。其中后 3 项是“已完成审查并
识别边界”，不表示 B 组问题已经实现：fault 仍存在 MM 锁内慢 I/O，可写 shared 映射因此被 C 组
显式拒绝，read-only shared 与 truncate 的完整失效契约仍待后续处理。
...
- [x] positioned I/O 不修改共享 offset；
- [x] FS/special-fd syscall 已完成分级；
- [x] 空壳 fd 型 syscall 已降级或具备完整最小生命周期；
- [x] epoll_pwait signal mask 不再被静默忽略；
- [x] epoll EFAULT、close 和 fd 复用行为有回归；
- [x] timerfd/memfd flags 与实际能力一致；
- [x] dup/fork 后 fd 与 File 共享关系正确；
- [x] sync 核心路径无可达 `todo!()`；
- [x] 不支持的同步能力不假成功；
- [x] create/unlink/rename 无明显旧缓存污染；
- [x] 打开后 unlink 的 fd 生命周期合理；
- [x] page cache dirty/version 语义稳定；
- [x] 至少一处锁内 I/O 热点得到改善；
- [x] 文件性能无明显回退；
